...
node --test "script/*.test.mjs"   # 数据工具测试（34 个，glob 必须带引号，裸目录形式在本机报错）
node --experimental-strip-types --test script/engine-state.test.ts script/pace-mode.test.ts script/goals.test.ts script/save.test.ts script/use-game.test.ts script/gameplay.test.ts script/verdict.test.ts script/family.test.ts script/talents.test.ts script/life-systems.test.ts script/weekly.test.ts script/achievements.test.ts script/undo.test.ts script/companion.test.ts script/personality.test.ts   # 引擎/档位/目标/存档/玩法/族谱/天赋/系统推导/周挑战/成就/回退/伴侣/性格测试（184 个，Node 22 直接跑 TS；npm test 已含全部）
npm run test:ui   # UI 组件测试（vitest + Testing Library，119 个）
...
- **goals.ts / achievements.ts / verdict.ts**：人生目标判定（6 预设 checkGoal）、成就判定（42 个 checkAchievements，铜/银/金三档 tier 仅展示分组，6 个 hidden 隐藏成就解锁前只露问号；applyAchievementBonus 每解锁 10 成就开局全属性 +2 封顶 +6；含性格成就与「改过自新」）、结局 key 纯函数（verdictKey，16 路线 flag + 5 档分数兜底；VERDICT_ROUTES 图鉴元数据表——2026-08 加第 14 条「铁窗人生 jailed」路线，TitleScreen/ShareCardModal 图鉴进度均为 VERDICT_ROUTES.length 动态化，SummaryScreen 结局标题同表查取）
...
- **推导系统（纯函数，零存档字段）**：jobs.ts（flag→职业映射 + jobLevel 从业每 3 年 1 级）、npcs.ts（history 中 family/love/friend 分类事件正负选择 → 家人/伴侣/朋友关系值 0-100）、gaokao.ts（学业 flag → 高考结果回顾；事件链已完整，本函数只做展示）、assets.ts（投资链 flag 递进 + 财富档 → 资产组合）、weekly.ts（ISO 周号 weekOf/weekSeed → 每周挑战目标 pickWeeklyGoal + checkWeeklyGoal 终局判定）、personas.ts（**人生过客**：6 位有名字的人物——发小阿凯/同桌林晓/初恋苏晴/挚友大鹏/贵人周老师/损友老马，PERSONAS 表含各自出场事件 id；personaBonds 从 history 中标注 `persona` 字段的事件正负选择推导好感 0-100、未出场保持 50；personaRelationText 五档文案；结算页「人生过客」人物卡只显示有互动的人物）、legacy.ts（**家族底蕴**：deriveLegacy 从族谱取最近 5 代手玩局终局属性均值，≥70 强项每项 +2 封顶 +6，代数 ≥2 生效；startNewGame 应用在传承之后、挑战之前；族谱面板展示）、personality.ts（**性格画像**：3 维 6 端——理性/感性/冒险/安稳/利己/利他；traitForOutcome 强信号规则从选项效果映射（智力≥6 且幸福≤0 → 理性、幸福≥6 且智力≤0 → 感性、负向与正向总额均≥8 → 冒险、财富≥8 且道德为负 → 利己、道德≥6 → 利他）+ PERSONA_FLAG_RULES 叙事 flag 补充表（gap_year/volunteer/civil_servant 等 19 个，内容层重写时扩充）；derivePersona 从 history 反查事件表逐条累积、缺失事件跳过；personaSummary 一句话概括（Top1 形容词 + 跨维 Top2 人设名词）；meetsPersonality 性格条件判定（事件 `conditions.minPersonality` 触发——`fragments/personality.json` 6 个性格专属事件 pers_0001-0006 各端一个，57-63 岁；lite 精简模式性格事件优先保留；checkConditions 与 sim-balance 均接入，仅带该条件的事件才计算画像，性能无感）。introSummary.ts（**选择回顾**：buildIntroSummary 从 history/snapshots 推导 0-12 岁大事记（效果绝对值前 4，按年龄升序）+ 属性成长（变化 ≥5 前 3）——童年定格面板数据源；buildCausalChain 同源取 13 岁起效果前 6 条——结算页「因果链」章节；共享 keyChoices helper）。**这些系统全部从 GameState 推导，旧存档与回看自动兼容**
- **主动行为系统（src/engine/activities.ts）**：8 个活动（健身/学习/打工/社交/体检/休闲/遛宠物/犯罪），活动表内嵌引擎（Activity{id/name/icon/desc/minAge/requires?/results[]}，每个结果池 3-4 个第二人称变体）；**每岁每个活动限 1 次**（GameState.actionsDone 可选字段 = 本岁已做活动 id 列表，MAKE_CHOICE 进入新岁自动重置——2026-08 平衡审计修复「一生一次」bug；旧存档兼容；不限制总次数）；活动**不推进年龄、不进 history**（不污染性格/传记/npc/人物推导）；MAKE_ACTION reducer（校验已做/minAge/requires/反馈页/autoPlay → 结果抽取 → applyOutcomes → flags 追加 → actionsDone push → feedback）；**犯罪**（minAge 14）：`crimeSuccessRate` = 55 + 运气×0.35 + 智力×0.25 钳位 [0,90]（2026-08 平衡审计调参：低属性新手风险更大，满属性仍可达钳位），`rollCrime(luck, int, rand)` 注入 rand 可测试——成功 wealth +12~15/morality -6~-8，失败 50% 被抓（**产出 jailed flag，无缝接入铁窗路线**）/50% 落荒而逃；UI：GameScreen「⚡ 行动」按钮 + ActionModal（可用性判定：年龄/宠物 flag/已做过，犯罪「高风险」角标）；**童年快进 0-12 岁**（introAuto 标记，2026-08 从 0-5 岁扩展）：普通手动局与每日/每周/种子挑战局均启用（快速模拟由 autoPlay 自带推进）——13 岁自动交还控制；推进由 hook 自动推进 effect 驱动（220ms 间隔 + 反馈 50ms 跳过），**挑战局固定选第一个选项（确定性，不破坏「同一天同一局」公平）**、普通局随机（GameScreen 不再点击翻页，DialogBox instant 立显）；MAKE_CHOICE 每岁只播 1 张幻灯片（findNextEvent introAuto 参数），事件效果/flags/成长照常执行；SKIP_INTRO ⏭ 一键跳过到 13 岁（同确定性规则）；快进结束弹**童年定格面板**（IntroSummaryModal：大事记/属性成长/关键选择，数据来自 buildIntroSummary）——旧存档 age<13 读档续播
...
- **每周挑战**：标题页「🗓️ 每周挑战」以周确定性种子（`weekSeed`，ISO 周号 → number）开局，同一周同一局 + 一个周目标（`pickWeeklyGoal` 从 9 个目标池按周种子抽取：活到 80/财富/学业/医生/家庭/桃李满门/性格鲜明/安稳一生/硬朗暮年），终局 `checkWeeklyGoal` 判定通关（`life-sim-weekly` 记录当周最佳与通关标记）；不写存档槽（isWeekly）、不开天赋构筑（公平固定开局）
...
- **GameScreen**：全屏流式布局（脱离 960×720 舞台）。场景 h-[55%] + 底部区 max-h-[45%] 恰好互补；**数值栏锚定场景区底缘（h-[55%] 容器内 flex 到底）——与底部区结构性不重叠，改动需保持此关系**；大屏限宽居中：数值栏网格 max-w-960、对话/选项/反馈内容 max-w-860；底部区含 `pb-9` 防速度按钮遮挡；**右上角操作组 = flex 容器自动排列**（↩️后悔/📌抉择/✕退出，半透明深底胶囊高对比，快速模拟只显示 ✕；改动按钮勿用绝对定位，防粘连）；✕ 确认弹窗三选项（取消/🔄 重新开始本局/确定回标题，存档保留；快速模拟不显示重开）；**反馈页「你的选择」徽章行**（lastTraits 从 history 最后一条反查选项推导，与 ChoicePanel 徽章同源）；**反馈页结构**：叙事行 + 数值行（带属性名，如「+8 智力」，引擎构造）+ 选后因果标注「这塑造了你：学识」（主导属性生效 ≥5 时，SHAPE_PHRASES 短语表；2026-08 选择影响可见化）；**童年定格面板**（IntroSummaryModal：introAuto 下降沿（13 岁）弹出，展示 buildIntroSummary 推导的童年大事记/属性成长/关键选择，「开始 13 岁的人生」确认后进入手动段）；**🧭 性格面板**（CharacterPanel 组件，数值栏按钮展开/收起：3 维 6 端进度 + 各端距专属际遇阈值还差几次/已解锁）；**🎯 目标进度**（顶部角标：预设目标 GOALS 简述 + checkGoal 达成标记，自定义目标逐项属性进度条）
- **DialogBox**：打字机效果（速度档位 + 点击跳过）+ 「▼ 点击继续」；事件标题显示为「标题」；事件文本经 useName 称呼替换（「你」→ 名字）
- **ChoicePanel**：选项按钮（`button.group` class）；**普通模式不显示效果数值**（2026-08 实测反馈：防按数值选择，选完反馈页显示精确变化），真实模式只显示 ↑/↓ 倾向箭头；选项文字 line-clamp-2 完整文本悬浮；**选项折叠**（默认只显示前 2 个 + 「· · · 更多选择（N）」展开，切换事件自动复位——2026-08 阅读减负）；**性格徽章**（traitForOutcome 推导，PERSONA_META 配色小标签，无信号不显示——留白也是区分；风格提示非数值，真实模式保留）
- **TitleScreen**：名字/性别 + **节奏档位（沉浸/精简）+ 打字速度 + 3 存档卡片 + 开局构筑（BuildModal 天赋抽卡 + 属性分配）+ 目标选择模态（GoalModal，含「🎯 自定义目标」勾选属性+滑杆设目标值 + 家族继承提示）+ 成就（AchievementsModal 铜/银/金分层 + 达成率进度条，隐藏成就 ❓）+ 人生图鉴（CollectionModal 16 结局路线收集，数据取自 stats.endings）/家族族谱（FamilyModal 跨世代收藏，行可点击回看）/生涯统计（StatsModal，含「每一世」回看列表）/玩法说明（GuideModal，首次进入自动弹出一次——`life-sim-guide-seen` 标记）入口 + 快捷入口行（⚡ 快速模拟/📅 每日挑战/🗓️ 每周挑战/📊 生涯/🏆 成就/📖 图鉴/🌳 家族/❓ 玩法/🔑 种子/🎨 主题，flex-wrap 窄屏换行）**。布局：不动层（光晕/粒子）+ 滚动层（内容 `my-auto` 居中——不溢出居中、溢出可滚动，杜绝 justify-center 对称裁切）；模态放滚动层外；**模态统一 `max-w-[92vw]` + `max-h-[min(520px,86vh)]` 防手机裁切**
- **SummaryScreen**：结算页（享年 + 结局 + 评分 + 属性 + 职业/高考/天赋推导信息行 + **结局判定依据**（verdictBasis 纯函数：命中路线 flag 或评分档，透明化）+ **性格注脚**（PERSONA_NOTES 按 Top1 性格端一句话）+ 成长曲线 + 大事记 + 目标达成度 + 与身边人（npcs 三线关系值）+ **性格画像（derivePersona 推导，3 维双端条形图 + personaSummary 一句话概括，弱画像只显示概括句）** + **人生际遇（personalityEncounters 纯函数：已触发的 pers_ 性格事件列表 + 性格达标未触发的提示，弱画像不显示）** + 资产组合 + 天赋传承面板（本局天赋设传承）+ 新解锁成就 + 本可发生而未触发 + 分享卡片 + 传记导出 + **🎫 人生名片（LifeCardModal：canvas 640×400 简历卡——结局/评分/性格/8 属性条/种子/日期，buildLifeCardData 纯函数可测试，下载 PNG）** + **人生年鉴（AlmanacModal：评分/曲线/职业资产/家人/大事记速览，导出 markdown）** + 每周挑战达成展示）
- **StatusBar/GameScreen**：属性网格上方职业/资产摘要行（jobStatus+assetStatus 推导 caption）；GameScreen 顶部每周挑战角标
- **SummaryScreen**：结算页（享年 + 结局 + 评分 + 属性 + **成长曲线（GrowthChart canvas 8 维随年龄折线图，图例悬停临时聚焦/点击固定聚焦单条曲线，其余淡化）+ 人生大事记完整时间线（里程碑 ⭐）+ **因果链**（buildCausalChain 推导的 13 岁起关键选择链，终点指向本局结局——「选择塑造人生」收束与重玩钩子）+ 目标达成度 + 新解锁成就 + 本可发生而未触发 + 分享卡片（ShareCardModal canvas PNG，含迷你成长曲线 + 名字行世代数「· 第 N 代」（App 按族谱最新一代传入）+ 底部种子码 + 传播 CTA「如果重来一次，你会怎么选？」）+ 传记导出**）
...
- 合并后密度校验：0-2 岁 3-5、3-12 岁 5-13、13-75 岁 3-9 个/岁（2026-08 入狱路线起上限 8→9，见 merge-fragments.mjs 的 checkDistribution）
