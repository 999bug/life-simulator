...
export default function App() {
  const { game, currentEvent, feedback, autoPlay, typeSpeed, saves, achievements, newAchievements, startGame, startAutoGame, makeChoice, continue: continue_, continueGame, reset, setTypeSpeed } = useGame();
  ...
  return (
    <div className="w-screen h-screen flex justify-center items-center bg-black overflow-hidden">
      <div className="w-full h-full max-w-[960px] max-h-[720px] relative overflow-hidden rounded-lg shadow-[0_0_80px_rgba(0,0,0,0.6)] text-white">
        {game.phase === 'title' && (
          <TitleScreen onStart={startGame} onAutoStart={startAutoGame} saves={saves} onContinue={continueGame} achievements={achievements} />
        )}
        {game.phase === 'playing' && (
          <GameScreen
            game={game}
        ...
        {game.phase === 'summary' && (
          <SummaryScreen game={game} onRestart={reset} newAchievements={newAchievements} />
        )}
      </div>
    </div>
  );
