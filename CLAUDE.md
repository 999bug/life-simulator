...
- **goals.ts / achievements.ts / verdict.ts**：人生目标判定（6 预设 checkGoal）、成就判定（39 个 checkAchievements，铜/银/金三档 tier 仅展示分组，6 个 hidden 隐藏成就解锁前只露问号；applyAchievementBonus 每解锁 10 成就开局全属性 +2 封顶 +6；含性格成就与「改过自新」）、结局 key 纯函数（verdictKey，14 路线 flag + 5 档分数兜底；VERDICT_ROUTES 图鉴元数据表——2026-08 加第 14 条「铁窗人生 jailed」路线，TitleScreen/ShareCardModal 图鉴进度均为 VERDICT_ROUTES.length 动态化，SummaryScreen 结局标题同表查取）
