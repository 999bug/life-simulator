...
- **主动行为系统（src/engine/activities.ts）**：8 个活动（健身/学习/打工/社交/体检/休闲/遛宠物/犯罪），活动表内嵌引擎（Activity{id/name/icon/desc/minAge/requires?/results[]}，每个结果池 3-4 个第二人称变体）；**每岁每个活动限 1 次**（GameState.actionsDone 可选字段 = 本岁已做活动 id 列表，旧存档兼容；不限制总次数）；活动**不推进年龄、不进 history**（不污染性格/传记/npc/人物推导）；MAKE_ACTION reducer（校验已做/minAge/requires/反馈页/autoPlay → 结果抽取 → applyOutcomes → flags 追加 → actionsDone push → feedback）；**犯罪**（minAge 14）：`crimeSuccessRate` = 60 + 运气×0.5 + 智力×0.3 钳位 [0,90]，`rollCrime(luck, int, rand)` 注入 rand 可测试——成功 wealth +12~15/morality -6~-8，失败 50% 被抓（**产出 jailed flag，无缝接入铁窗路线**）/50% 落荒而逃；UI：GameScreen「⚡ 行动」按钮 + ActionModal（可用性判定：年龄/宠物 flag/已做过，犯罪「高风险」角标）；**幼儿期走过场**（SKIP_INTRO）：普通手动局（非快速模拟/每日/每周/种子——每日/每周/种子挑战自动随机选择会破坏「同一天同一局」公平性）0-5 岁自动播放（RuntimeState.introAuto 标记），MAKE_CHOICE 推进到 6 岁自动交还控制，GameScreen 显示「👶 幼儿期 · 自动播放」角标 + ⏭ 跳过
