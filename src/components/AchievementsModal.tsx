...
/** 成就总览模态（标题页入口）：按铜/银/金分层展示，隐藏成就解锁前只露问号 */
      ...
      <div className="w-[480px] max-w-[92vw] max-h-[min(520px,86vh)] overflow-y-auto rounded-2xl border border-white/10 bg-[#15152a] p-6 flex flex-col gap-3.5" onClick={e => e.stopPropagation()}>
        ...
        <div className="mx-auto w-[240px]">
          <div className="h-[5px] bg-white/8 rounded-sm overflow-hidden">
            <div className="h-full rounded-sm bg-[#c9a96e] transition-all duration-700" style={{ width: `${pct}%` }} />
          </div>
          <p className="text-center text-[10px] text-white/30 mt-1.5 tracking-[2px]">达成率 {pct}%</p>
        </div>
        {tiers.map(tier => {
          ...
          return (
            <div key={tier} className="flex flex-col gap-2">
              ...
              {list.map(a => {
                ...
                // 隐藏成就未解锁：名称/描述只露问号（保留探索乐趣）
                const name = got || !a.hidden ? a.name : '？？？';
                const desc = got || !a.hidden ? a.desc : a.hidden ? '条件不公开，等你亲手解开' : '';
                return (
                  <div key={a.id} className={`flex items-start gap-3 p-3 rounded-lg border ${got ? 'border-[#c9a96e]/25 bg-[#c9a96e]/5' : 'border-white/[0.06] bg-white/[0.02] opacity-50'}`}>
                    <span className="text-[16px] leading-none mt-0.5">{got ? a.icon : a.hidden ? '❓' : '🔒'}</span>
                    <div className="flex-1">
                      <div className={`text-[13px] ${got ? 'text-[#c9a96e]' : 'text-white/50'}`}>{name}</div>
                      <div className="text-[11px] text-white/35 mt-0.5 leading-relaxed">{desc}</div>
                    </div>
                    {a.hidden && !got && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full mt-0.5 text-[#b57edc]/60 bg-[#b57edc]/10">隐藏</span>
                    )}
                    <span
